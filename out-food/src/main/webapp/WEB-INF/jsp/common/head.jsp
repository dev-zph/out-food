...
<div class="header">
			...
			<div class="rigister-info">
				<ul>
					<div class="welcome fl" style="font-size: 20px; font-weight: 600">
						欢迎来平台订餐
						<c:if test="${user ne null}">${user.nickName}<li><a
								href="${ctx}/toExit.html">安全退出</a></li>
						</c:if>
	...
	<div class="bottom-header">
		<div class="container">
			...
			<div class="logo">
				<a href="${ctx} "><img src="${ctx}/resources/images/logo.png"
					title="gaia" /></a>
			</div>
			<div class="bottom-header-left">
				<ul>
					...
					<c:if test="${shop != null}">
						<div class="top-menu-hd">
							<a href="${ctx}/item/get/1.html"
								<c:if test='${user ne null}'></c:if>>商铺管理</a>
						</div>
					</c:if>
					<c:if test="${shop eq null}">
						<div class="top-menu-hd">
							<a href="${ctx}/navigate_apply/index.html"
								<c:if test='${user ne null}'></c:if>>我要开店</a>
						</div>
					</c:if>
					<div class="top-menu-hd">
						<a href="${ctx}/SenderOrder/getOrderList.html"
