...
public class PageAjax<T> {
	...
	public void initPageBar() {
		...
		if (totalPageNo > 0) {
			...
			pb.append("<a href=\"javascript:void(0)\" class=\"previous\" onclick=\"toPageAjax('"
			        + this.firstPageNo + "','" + this.pageUrl + "');\">首页</a>");
			pb.append("<a href=\"javascript:void(0)\" class=\"index\" onclick=\"toPageAjax('"
			...
			if (totalPageNo <= seriesSize) {
				for (int i = 1; i <= totalPageNo; i++) {
					if (this.pageNo == i) {
						...
						pb.append("<a href=\"javascript:void(0)\" onclick=\"toPageAjax('"
						        + i
						        + "','"
						        + this.pageUrl
						        + "');\">"
			...
			// pb.append("function toPageAjax(pageNo,pageUrl){");
	 ...
	 * @param reqPageNo
	 */
	public PageAjax(int totalRecords, int reqPageNo, String pageUrl) {
